...
void gf_3vect_dot_prod_sve_intrinsics(int len, int vlen, unsigned char *gftbls, 
                                      unsigned char **src, unsigned char **dest)
{
    ...
    while (pos < len) {
        ...
        for (int v = 0; v < vlen; v++) {
            ...
            // Destination 2
            unsigned char *tbl_base2 = &gftbls[2 * vlen * 32 + v * 32];
            svuint8_t tbl_lo2 = svld1_u8(svptrue_b8(), tbl_base2);
            svuint8_t tbl_hi2 = svld1_u8(svptrue_b8(), tbl_base2 + 16);
            svuint8_t gf_lo2 = svtbl_u8(tbl_lo2, src_lo);
            svuint8_t gf_hi2 = svtbl_u8(tbl_hi2, src_hi);
            svuint8_t gf_result2 = sveor_x(pg, gf_lo2, gf_hi2);
            dest_acc2 = sveor_x(pg, dest_acc2, gf_result2);
        }
